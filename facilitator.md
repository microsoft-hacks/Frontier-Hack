...
- [`deploy-search-electric-plant.ps1`](scripts/deploy-search-electric-plant.ps1) deploys the shared Azure AI Search service (default `search-hack-shared`) into a separate resource group and uploads the Electric Plant knowledge base into the `electric-plant-maintenance` index by running `labs/electric-plant/search-knowledge/setup_index.py`. The Facilitation track of Challenge 1 then requires the service to be attached to the shared project as a connection named exactly `search-hack-shared`.
